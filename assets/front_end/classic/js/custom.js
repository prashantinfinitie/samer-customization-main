...
/* ========================================
   ...
   ======================================== */
console.log('=== Location Seller Discovery JS Loaded (Classic Theme) ===');
...
var LocationSellerDiscovery = (function() {
    ...
    function init() {
        ...
        if (elements.detectLocationBtn.length === 0) {
            console.log('LocationSellerDiscovery: Not on sellers page, skipping');
            return;
        }
        
        console.log('LocationSellerDiscovery: Found detect button, initializing');
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response && response.address) {
                    ...
                    if (locationName) {
                        elements.locationText.html('<i class="fa fa-check-circle text-success mr-1"></i>' + locationName);
                    } else {
                        elements.locationText.html('<i class="fa fa-check-circle text-success mr-1"></i>Location set');
                    }
                }
            },
            error: function() {
    ...
    function setLoading(isLoading) {
        ...
        if (isLoading) {
            ...
            elements.detectLocationBtn.find('i').removeClass('fa-crosshairs uil-crosshair').addClass('fa-spinner fa-spin');
        } else {
            elements.detectLocationBtn.removeClass('loading').prop('disabled', false);
            elements.detectLocationBtn.find('i').removeClass('fa-spinner fa-spin').addClass('fa-crosshairs');
        }
    }
