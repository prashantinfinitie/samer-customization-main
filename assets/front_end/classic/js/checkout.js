...
function fetchShippingQuotes(address_id) {
    ...
    $.ajax({
        ...
        dataType: 'json',
        beforeSend: function () {
            $('#quotes_container').html('<div class="text-center"><i class="fa fa-spinner fa-spin"></i> Loading shipping options...</div>');
        },
...
/**
 * Display shipping company quotes
 */
function displayShippingQuotes(quotes) {
    ...
    quotes.forEach(function (quote, index) {
            ...
            try {
                ...
                if (Array.isArray(parsed)) {
                    ...
                    additional_charges = Object.keys(parsed).map(function (key) {
                        return { name: key, amount: parsed[key] };
                    });
                }
            } catch (e) {
                additional_charges = [];
        ...
        html += '<div class="detail-row"><span class="detail-label">Delivery:</span><span class="detail-value">' + quote.eta_text + ' days</span></div>';
        html += '<div class="detail-row"><span class="detail-label">Base Price:</span><span class="detail-value">' + currency + price.toFixed(2) + '</span></div>';
    ...
    if (quotes.length > 0) {
        ...
        selectShippingQuote(firstQuote.id, firstQuote.shipping_company_id, firstPrice + firstAdditional, firstQuote.cod_available);
    }
}
...
$(document).on('change', '.shipping-option-radio', function () {
    ...
    var company_id = $radio.attr('data-company-id');
    var price = parseFloat($radio.attr('data-price'));
    var cod_available = $radio.attr('data-cod');
...
function selectShippingQuote(quote_id, company_id, price, cod_available) {
    ...
    $('.delivery_charge_with_cod').html(price.toFixed(2)).val(price);
    $('.delivery_charge_without_cod').html(price.toFixed(2)).val(price);

    // Update final total
    updateFinalTotalWithQuote();
 ...
 */
function updateFinalTotalWithQuote() {
    var sub_total = parseFloat($('#sub_total').val().replace(',', '')) || 0;
    var delivery_charge = parseFloat($('.delivery_charge_without_cod').val()) || 0;
    if (typeof delivery_charge === 'string') {
        delivery_charge = parseFloat(delivery_charge.replace(',', '')) || 0;
    }
    ...
    if (selected_payment === 'COD') {
        delivery_charge = parseFloat($('.delivery_charge_with_cod').val()) || 0;
        if (typeof delivery_charge === 'string') {
            delivery_charge = parseFloat(delivery_charge.replace(',', '')) || 0;
        }
    }
...
// Update final total when payment method changes (for shipping quotes)
$(document).on('change', 'input[name="payment_method"]', function () {
    if ($('#provider_type').val() === 'company') {
        updateFinalTotalWithQuote();
    }
});

// Update final total when wallet checkbox changes (for shipping quotes)
$(document).on('change', '#wallet_balance', function () {
    if ($('#provider_type').val() === 'company') {
        setTimeout(updateFinalTotalWithQuote, 100);
    }
});
