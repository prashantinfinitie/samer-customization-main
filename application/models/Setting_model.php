...
class Setting_model extends CI_Model
{
    ...
    public function update_shipping_company_privacy_policy($data)
    {
        $data = escape_array($data);
        $query = $this->db->get_where('settings', array(
            'variable' => 'shipping_company_privacy_policy'
        ));
        $count = $query->num_rows();
        if ($count === 0) {
            $data = array(
            ...
            $this->db->set('value', $data['privacy_policy_input_description'])->where('variable', 'shipping_company_privacy_policy')->update('settings');
        }
    }
    ...
    public function update_shipping_company_terms_n_condtions($data)
    {
        $data = escape_array($data);
        $query = $this->db->get_where('settings', array(
            'variable' => 'shipping_company_terms_conditions'
        ));
        $count = $query->num_rows();
        if ($count === 0) {
            $data = array(
            ...
            $this->db->set('value', $data['terms_n_conditions_input_description'])->where('variable', 'shipping_company_terms_conditions')->update('settings');
        }
    }
