...
class Fund_transfers_model extends CI_Model
{
    ...
    function set_fund_transfer_shipping_company($shipping_company_id, $amount, $opening_bal, $status = 'success', $message = "")
    {
        $t = &get_instance();
        $res = $t->db->select('balance')->where('id', $shipping_company_id)->get('users')->result_array();
    ...
    function get_fund_transfers_list_shipping_company($user_id = '')
    {
        $offset = 0;
        $limit = 10;
        $sort = 'id';
        ...
        if ($user_id != '' && is_numeric($user_id)) {
            $where = array('fund_transfers.shipping_company_id' => trim($user_id));
        }
        ...
        $count_res->join('users', ' fund_transfers.shipping_company_id = users.id ');
        $transfers_count = $count_res->get('fund_transfers')->result_array();
        $total = isset($transfers_count[0]['total']) ? $transfers_count[0]['total'] : 0;
        ...
        $search_res->join('users', 'fund_transfers.shipping_company_id = users.id', 'left');
        $transfers_res = $search_res->order_by($sort, $order)->limit($limit, $offset)->get('fund_transfers')->result_array();
