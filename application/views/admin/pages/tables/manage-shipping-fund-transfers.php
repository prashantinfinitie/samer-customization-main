                            ...
                            <select id="filter_company" class="form-control">
                                ...
                                <?php if (isset($shipping_companies) && !empty($shipping_companies)) : ?>
                                    <?php foreach ($shipping_companies as $sc) : ?>
                                        <option value="<?= $sc['id'] ?>"><?= $sc['username'] ?></option>
                                    <?php endforeach; ?>
                                <?php endif; ?>
                            </select>
                        </div>
        ...
        <form id="fund_transfer_form" class="form-horizontal" action="<?= base_url('admin/shipping_companies/manage_fund_transfer'); ?>" method="POST">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="alert alert-info" id="ft_payout_info_box">
                        <h6 class="mb-2"><i class="fa fa-calculator"></i> Payout Summary (Prepaid Orders)</h6>
                        <div class="row">
                            <div class="col-md-3">
                                <small class="text-muted">Delivered Orders</small>
                                <div class="font-weight-bold" id="ft_payout_order_count">-</div>
                            </div>
                            <div class="col-md-3">
                                <small class="text-muted">Total Earnings</small>
                                <div class="font-weight-bold text-success" id="ft_payout_total_earnings">-</div>
                            </div>
                            <div class="col-md-3">
                                <small class="text-muted">Already Paid</small>
                                <div class="font-weight-bold text-secondary" id="ft_payout_total_paid">-</div>
                            </div>
                            <div class="col-md-3">
                                <small class="text-muted">Pending Amount</small>
                                <div class="font-weight-bold text-danger" id="ft_payout_pending_amount">-</div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="row">
                        <div class="form-group col-md-6">
                            ...
                            <small class="text-muted">Click "Pay Pending" to auto-fill</small>
                        </div>
                        <div class="form-group col-md-6">
                            <label>Date</label>
                    ...
                    <div class="row">
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <textarea name="message" id="ft_message" class="form-control" rows="1">Admin payout for delivery charges</textarea>
                        </div>
                    </div>
                ...
                <div class="modal-footer d-flex justify-content-between">
                    <button type="button" class="btn btn-outline-info" id="ft_btn_pay_pending">Pay Pending Amount</button>
                    <div>
...
<script>
    ...
    $(document).ready(function() {
        ...
        $(document).on('click', '.fund_transfer_shipping_company', function(e) {
            ...
            $('#ft_company_name').val('Loading...');
            $('#ft_admin_balance').val('Loading...');
            $('#ft_payout_order_count').text('-');
            $('#ft_payout_total_earnings').text('-');
            $('#ft_payout_total_paid').text('-');
            $('#ft_payout_pending_amount').text('-');
            ftCurrentPendingAmount = 0;

            // Fetch all payout details in one call
            $.ajax({
                url: "<?= base_url('admin/shipping_companies/get_company_payout_details') ?>/" + companyId,
                method: "GET",
                dataType: "json",
                success: function(res) {
                    if (res.error) {
                        toastr.error(res.message || 'Failed to load company details');
                        $('#ft_company_name').val('Error');
                        return;
                    }

                    // Company info
                    $('#ft_company_name').val(res.company.username + ' (ID: ' + res.company.id + ')');

                    // Admin balance
                    $('#ft_admin_balance').val(parseFloat(res.admin_balance).toFixed(2));

                    // Payout summary
                    if (res.payout) {
                        $('#ft_payout_order_count').text(res.payout.order_count);
                        $('#ft_payout_total_earnings').text('₹' + parseFloat(res.payout.total_earnings).toFixed(2));
                        $('#ft_payout_total_paid').text('₹' + parseFloat(res.payout.total_paid).toFixed(2));
                        $('#ft_payout_pending_amount').text('₹' + parseFloat(res.payout.pending_amount).toFixed(2));
                        ftCurrentPendingAmount = parseFloat(res.payout.pending_amount);
                    }
                },
                error: function() {
                    $('#ft_company_name').val('Error loading');
        ...
        $(document).on('click', '#ft_btn_pay_pending', function(e) {
            e.preventDefault();
            if (ftCurrentPendingAmount > 0) {
                $('#ft_amount').val(ftCurrentPendingAmount.toFixed(2));
            } else {
