                                        ...
                                        <ul class="list-group bg-grey move order-container" id="sortable">
                                            ...
                                            ?>
                                                    <li class="list-group-item d-flex bg-gray-light align-items-center h-25" id="category_id-<?= $row['id'] ?>">
                                                        ...
                                                        <div class="col-3"><span><?php 
                                                            // Check locale and Arabic field - only apply notranslate when locale is Arabic AND Arabic field exists
                                                            $locale = get_current_locale();
                                                            $has_arabic = !empty($row['name_ar']);
                                                            $use_notranslate = ($locale === 'ar' && $has_arabic);
                                                            $category_name = output_escaping($row['name']);
                                                            if ($use_notranslate) {
                                                                // Arabic locale and Arabic field exists - wrap in notranslate
                                                                echo '<span class="notranslate">' . $category_name . '</span>';
                                                            } else {
