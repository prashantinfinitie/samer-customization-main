...
*/
    ...
    public function __construct()
    {
        ...
        $this->load->model(['order_model', 'category_model', 'transaction_model', 'Home_model', 'customer_model', 'ticket_model', 'delivery_boy_model', 'Seller_model', 'Shipping_company_model', 'Fund_transfers_model', 'Area_model']);
        $this->load->helper([]);
    ...
    */
    ...
    /*
    ---------------------------------------------------------------------------
    SHIPPING COMPANY MANAGEMENT APIs (for Admin App)
    ---------------------------------------------------------------------------
    */
     ...
     *
     * Parameters:
     * - id: (optional) specific company ID
     * - search: (optional) search keyword
     * - limit: (optional, default 25)
     * - offset: (optional, default 0)
     * - sort: (optional, default 'id')
     * - order: (optional, default 'DESC')
     * - status: (optional) 0=pending, 1=approved
     */
    public function get_shipping_companies()
    {
        ...
        $id = (isset($_POST['id']) && is_numeric($_POST['id'])) ? $this->input->post('id', true) : "";
        ...
        $sort = (isset($_POST['sort']) && !empty($_POST['sort'])) ? $_POST['sort'] : 'u.id';
        ...
        $this->db->where('ug.group_id', 6); // shipping_company group
        ...
        foreach ($companies as $row) {
            unset($row['password']);
            $row['image'] = !empty($row['image']) && file_exists(FCPATH . $row['image']) ? base_url($row['image']) : base_url() . NO_USER_IMAGE;

            // Get serviceable areas
            $row['serviceable_zipcodes_list'] = [];
            if (!empty($row['serviceable_zipcodes'])) {
                $zipcodes = explode(',', $row['serviceable_zipcodes']);
                $row['serviceable_zipcodes_list'] = $this->db->select('id, zipcode')->where_in('id', $zipcodes)->get('areas')->result_array();
            }
            $row['serviceable_cities_list'] = [];
            if (!empty($row['serviceable_cities'])) {
                $cities = explode(',', $row['serviceable_cities']);
                $row['serviceable_cities_list'] = $this->db->select('id, name')->where_in('id', $cities)->get('cities')->result_array();
            }
            $rows[] = $row;
        }

        $this->response['error'] = false;
        $this->response['message'] = 'Shipping companies retrieved successfully';
    ...
    /**
     * Update shipping company status (approve/reject)
     *
     * Parameters:
     * - company_id: required
     * - status: required (0=pending, 1=approved)
     */
    public function update_shipping_company_status()
    {
        if (!$this->verify_token()) {
            return false;
        }

        $this->form_validation->set_rules('company_id', 'Company ID', 'trim|numeric|required|xss_clean');
        ...
        // Verify it's a shipping company
        $company = $this->db->select('u.id, u.username, u.email')
            ->from('users u')
        ...
        if (empty($company)) {
            $this->response['error'] = true;
            $this->response['message'] = 'Shipping company not found';
            print_r(json_encode($this->response));
            return;
        }

        $this->db->where('id', $company_id)->update('users', ['status' => $status]);

        $this->response['error'] = false;
        $this->response['message'] = $status == 1 ? 'Shipping company approved' : 'Shipping company status updated';
        $this->response['data'] = [];
        print_r(json_encode($this->response));
    }

    /**
     * Assign shipping company to order
     *
     * Parameters:
     * - order_id: required
     * - shipping_company_id: required
     */
    public function assign_shipping_company()
    {
        if (!$this->verify_token()) {
            return false;
        }
        ...
        $this->form_validation->set_rules('shipping_company_id', 'Shipping Company ID', 'trim|numeric|required|xss_clean');
        ...
        // Verify shipping company exists and is approved
        $company = $this->db->select('u.id, u.status, u.fcm_id')
            ->from('users u')
            ->join('users_groups ug', 'ug.user_id = u.id')
            ->where('u.id', $shipping_company_id)
            ->where('ug.group_id', 6)
            ->where('u.status', 1)
            ->get()->row_array();
        ...
        if (empty($company)) {
            $this->response['error'] = true;
            $this->response['message'] = 'Shipping company not found or not approved';
            print_r(json_encode($this->response));
            return;
        }

        // Assign to order
        $this->db->where('id', $order_id)->update('orders', ['shipping_company_id' => $shipping_company_id]);

        // Send notification to shipping company
        if (!empty($company['fcm_id'])) {
            $fcmMsg = array(
                'title' => "New Order Assigned",
                'body' => "Order #$order_id has been assigned to you for delivery",
                'type' => "order_assigned",
            );
            // send_notification(...);
        }
        ...
        $this->response['error'] = false;
        $this->response['message'] = 'Shipping company assigned successfully';
        $this->response['data'] = [];
        print_r(json_encode($this->response));
    }

    /**
     * Collect cash from shipping company (for COD orders)
     *
     * Parameters:
     * - shipping_company_id: required
     * - amount: required
     * - message: optional
     */
    public function collect_shipping_company_cash()
    {
        if (!$this->verify_token()) {
            return false;
        }
        ...
        $this->form_validation->set_rules('shipping_company_id', 'Shipping Company ID', 'trim|numeric|required|xss_clean');
        $this->form_validation->set_rules('amount', 'Amount', 'trim|numeric|required|greater_than[0]|xss_clean');
        ...
        $message = $this->input->post('message', true) ?: 'Cash collected by admin';

        // Verify shipping company and get cash balance
        $company = fetch_details('users', ['id' => $company_id], 'id, username, cash_received');
        if (empty($company)) {
            $this->response['error'] = true;
            $this->response['message'] = 'Shipping company not found';
            print_r(json_encode($this->response));
        ...
        $cash_in_hand = floatval($company[0]['cash_received']);
        if ($amount > $cash_in_hand) {
            $this->response['error'] = true;
            $this->response['message'] = 'Amount exceeds cash in hand (' . $cash_in_hand . ')';
            print_r(json_encode($this->response));
            return;
        }

        // Deduct from shipping company's cash_received
        $this->db->set('cash_received', 'cash_received - ' . $amount, FALSE)
        ...
        $this->response['error'] = false;
        $this->response['message'] = 'Cash collected successfully';
        $this->response['data'] = [
            'new_cash_balance' => strval($cash_in_hand - $amount)
        ];
        print_r(json_encode($this->response));
    }

    /**
     * Transfer funds to shipping company (for prepaid orders)
     *
     * Parameters:
     * - shipping_company_id: required
     * - amount: required
     * - message: optional
     */
    public function transfer_to_shipping_company()
    {
        if (!$this->verify_token()) {
            return false;
        }
        ...
        $company_id = $this->input->post('shipping_company_id', true);
        $amount = floatval($this->input->post('amount', true));
        $message = $this->input->post('message', true) ?: 'Fund transfer from admin';
        ...
        // Get admin user ID from token/session
        $admin_id = isset($_SESSION['user_id']) ? $_SESSION['user_id'] : 1;
        $admin = fetch_details('users', ['id' => $admin_id], 'balance');
        $admin_balance = isset($admin[0]['balance']) ? floatval($admin[0]['balance']) : 0;

        if ($amount > $admin_balance) {
            $this->response['error'] = true;
            $this->response['message'] = 'Insufficient admin balance';
            print_r(json_encode($this->response));
            return;
        }
        ...
        $this->db->set('balance', 'balance - ' . $amount, FALSE)
            ->where('id', $admin_id)->update('users');

        // Add to shipping company balance
        $this->db->set('balance', 'balance + ' . $amount, FALSE)
            ->where('id', $company_id)->update('users');

        // Record fund transfer
        $this->load->model('Fund_transfers_model');
        $this->Fund_transfers_model->set_fund_transfer_shipping_company($company_id, $amount, $message);

        $this->response['error'] = false;
        $this->response['message'] = 'Fund transferred successfully';
        $this->response['data'] = [];
        print_r(json_encode($this->response));
    }

    /**
     * Get shipping company payout info (pending amount for prepaid orders)
     *
     * Parameters:
     * - shipping_company_id: required
     */
    public function get_shipping_company_payout_info()
    {
        if (!$this->verify_token()) {
            return false;
        }
        ...
        $payout_info = $this->Shipping_company_model->get_pending_payout($company_id);

        $company = fetch_details('users', ['id' => $company_id], 'username, balance, cash_received');

        $this->response['error'] = false;
        $this->response['message'] = 'Payout info retrieved successfully';
        $this->response['data'] = [
            'company_name' => isset($company[0]['username']) ? $company[0]['username'] : '',
            'current_balance' => isset($company[0]['balance']) ? $company[0]['balance'] : '0',
            'cash_in_hand' => isset($company[0]['cash_received']) ? $company[0]['cash_received'] : '0',
    ...
    /**
     * Get shipping company cash collection history
     *
     * Parameters:
     * - shipping_company_id: optional (all if not provided)
     * - limit: optional (default 25)
     * - offset: optional (default 0)
     */
    public function get_shipping_company_cash_collection()
    {
        if (!$this->verify_token()) {
            return false;
        }
        ...
        $company_id = (isset($_POST['shipping_company_id']) && is_numeric($_POST['shipping_company_id'])) ? $this->input->post('shipping_company_id', true) : null;
        $limit = (isset($_POST['limit']) && is_numeric($_POST['limit'])) ? $this->input->post('limit', true) : 25;
        $offset = (isset($_POST['offset']) && is_numeric($_POST['offset'])) ? $this->input->post('offset', true) : 0;
        ...
        if ($company_id) {
            $this->db->where('t.user_id', $company_id);
        }
        ...
        foreach ($transactions as $row) {
            $rows[] = [
                ...
                'amount' => $row['amount'],
                'type' => $row['type'],
                'type_label' => ($row['type'] == 'shipping_company_cash') ? 'Received by Company' : 'Collected by Admin',
                'message' => $row['message'],
        ...
        $this->response['error'] = false;
        $this->response['message'] = 'Cash collection history retrieved';
        $this->response['total'] = strval($total);
        $this->response['data'] = $rows;
        print_r(json_encode($this->response));
    }
    ...
    /**
     * Get shipping company fund transfers history
     *
     * Parameters:
     ...
     * - offset: optional (default 0)
     */
    public function get_shipping_company_fund_transfers()
    {
        if (!$this->verify_token()) {
            return false;
        }
        ...
        $this->db->select('ft.*, u.username, u.mobile');
        $this->db->from('fund_transfers ft');
        $this->db->join('users u', 'u.id = ft.shipping_company_id');
        $this->db->where('ft.shipping_company_id IS NOT NULL');

        if ($company_id) {
            $this->db->where('ft.shipping_company_id', $company_id);
        }
        ...
        $this->response['error'] = false;
        $this->response['message'] = 'Fund transfers retrieved';
        $this->response['total'] = strval($total);
        $this->response['data'] = $transfers;
        print_r(json_encode($this->response));
    }

    /**
     * Get orders assigned to shipping companies
     *
     * Parameters:
     * - shipping_company_id: optional
     * - status: optional
     ...
     * - offset: optional (default 0)
     */
    public function get_shipping_company_orders()
    {
        if (!$this->verify_token()) {
            return false;
        }
        ...
        $company_id = (isset($_POST['shipping_company_id']) && is_numeric($_POST['shipping_company_id'])) ? $this->input->post('shipping_company_id', true) : null;
        $status = (isset($_POST['status']) && !empty($_POST['status'])) ? $this->input->post('status', true) : null;
        $limit = (isset($_POST['limit']) && is_numeric($_POST['limit'])) ? $this->input->post('limit', true) : 25;
        $offset = (isset($_POST['offset']) && is_numeric($_POST['offset'])) ? $this->input->post('offset', true) : 0;

        $this->db->select('o.*, u.username as customer_name, u.mobile as customer_mobile,
                           sc.username as shipping_company_name, sc.mobile as shipping_company_mobile,
                           a.address, a.city as address_city, a.pincode as address_pincode');
        ...
        $this->db->where('o.shipping_company_id IS NOT NULL');

        if ($company_id) {
            $this->db->where('o.shipping_company_id', $company_id);
        }
        if ($status) {
            $this->db->where('o.active_status', $status);
        }
