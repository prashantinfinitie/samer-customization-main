...
class Product extends CI_Controller
{
    ...
    public function create_product()
    {
        ...
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            // Load all sellers for both add and edit
            ...
            if (isset($_GET['edit_id']) && !empty($_GET['edit_id'])) {
                ...
                // Keep seller data for backward compatibility
    ...
    public function add_product()
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            if (!$this->form_validation->run()) {
                ...
                // Handle store_id - if seller_id is provided, get default store for that seller
                if (isset($_POST['seller_id']) && !empty($_POST['seller_id'])) {
                    $this->load->model('Store_model');
                    if (isset($_POST['store_id']) && !empty($_POST['store_id'])) {
                        // Use provided store_id if it belongs to the seller
                        $store = $this->Store_model->get_store($_POST['store_id'], $_POST['seller_id']);
                        if (!empty($store)) {
                            $product_data['store_id'] = $_POST['store_id'];
                        } else {
                            // If store doesn't belong to seller, get default store
                            $default_store = $this->Store_model->get_default_store($_POST['seller_id']);
                            $product_data['store_id'] = !empty($default_store) ? $default_store['id'] : null;
                        }
                    } else {
                        // Get default store for the seller
                        $default_store = $this->Store_model->get_default_store($_POST['seller_id']);
                        $product_data['store_id'] = !empty($default_store) ? $default_store['id'] : null;
                    }
                }
    ...
    public function view_product()
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            if (isset($_GET['edit_id']) && !empty($_GET['edit_id'])) {
                ...
                // Apply locale transformation to product details
                $locale = get_current_locale();
                if (!empty($res['product'])) {
                    foreach ($res['product'] as &$product) {
                        // Preserve original Arabic fields before transformation for conditional notranslate check
                        $original_name_ar = $product['name_ar'] ?? null;
                        $original_short_desc_ar = $product['short_description_ar'] ?? null;
                        $original_desc_ar = $product['description_ar'] ?? null;
                        $original_category_name_ar = $product['category_name_ar'] ?? null;

                        $product = apply_locale_to_product($product, $locale);

                        // Restore original Arabic fields for conditional notranslate check in view
                        if ($original_name_ar !== null) {
                            $product['name_ar'] = $original_name_ar;
                        }
                        if ($original_short_desc_ar !== null) {
                            $product['short_description_ar'] = $original_short_desc_ar;
                        }
                        if ($original_desc_ar !== null) {
                            $product['description_ar'] = $original_desc_ar;
                        }
                        if ($original_category_name_ar !== null) {
                            $product['category_name_ar'] = $original_category_name_ar;
                        }
                    }
                }
                ...
                $this->data['locale'] = $locale; // Pass locale to view for conditional notranslate

                $category_result = fetch_details('categories', ['status' => '1'], 'id,name,name_ar');
                // Apply locale transformation to categories
                if (!empty($category_result)) {
                    foreach ($category_result as &$cat) {
                        $cat = apply_locale_to_category($cat, $locale);
                    }
                }
                $this->data['category_result'] = $category_result;
                if (!empty($res['product'])) {
                    $this->load->view('admin/template', $this->data);
                } else {
