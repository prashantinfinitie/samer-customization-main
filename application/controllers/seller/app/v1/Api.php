    ...
    */
    public function get_order_items()
    {
        ...
        if (!$this->form_validation->run()) {
            ...
            // Clean HTML tags from product names in order items for API response
            // fetch_order_items() uses apply_locale_to_product() which adds HTML tags - we need clean text for APIs
            if (!empty($order_details['order_data'])) {
                foreach ($order_details['order_data'] as &$order) {
                    if (isset($order['order_items']) && is_array($order['order_items']) && !empty($order['order_items'])) {
                        foreach ($order['order_items'] as &$item) {
    ...
    public function get_categories_list()
    {
        ...
        if ($lang == 'ar' && !empty($cat_res)) {
            ...
            if (isset($cat_res['rows']) && !empty($cat_res['rows'])) {
                // Handle rows structure - could be array or object with numeric keys
                if (is_array($cat_res['rows'])) {
                    // Process each category in rows
                    $processed_rows = [];
                    foreach ($cat_res['rows'] as $key => $category) {
                        // Process single category
                        if (is_array($category) || is_object($category)) {
                            $category_array = is_object($category) ? (array)$category : $category;
                            $processed_category = $this->apply_category_language([$category_array], $lang);
                            $processed_rows[$key] = !empty($processed_category) ? $processed_category[0] : $category;
                        } else {
                            $processed_rows[$key] = $category;
                        }
                    }
                    $cat_res['rows'] = $processed_rows;
                }
            } elseif (isset($cat_res['data']) && is_array($cat_res['data'])) {
                $cat_res['data'] = $this->apply_category_language($cat_res['data'], $lang);
            } elseif (is_array($cat_res)) {
    ...
    public function get_statistics()
    {
        ...
        */
        ...
        $user_id = isset($this->user_details['id']) && $this->user_details['id'] !== null ? $this->user_details['id'] : '';
        $lang = get_api_language();
        $res = $this->db->select('c.name as name,c.name_ar as name_ar,count(c.id) as counter')->where(['p.status' => '1', 'c.status' => '1', 'p.seller_id' => $user_id])->join('products p', 'p.category_id=c.id')->group_by('c.id')->get('categories c')->result_array();
        foreach ($res as $row) {
            // Use Arabic name if lang=ar and name_ar exists, else use English name
            $cat_name = ($lang == 'ar' && !empty($row['name_ar'])) ? $row['name_ar'] : $row['name'];
            $tempRow['cat_name'][] = $cat_name;
    ...
    public function add_products()
    {
        /*
            ...
            pro_input_name_ar: product name in Arabic (optional)
            short_description: description
            short_description_ar: short description in Arabic (optional)
            ...
            pro_input_description_ar:product's description in Arabic (optional)
       ...
       */
        ...
        $this->form_validation->set_rules('pro_input_name_ar', 'Product Name (Arabic)', 'trim|xss_clean');
        $this->form_validation->set_rules('short_description', 'Short Description', 'trim|required');
        $this->form_validation->set_rules('short_description_ar', 'Short Description (Arabic)', 'trim|xss_clean');
        ...
        $this->form_validation->set_rules('pro_input_description_ar', 'Description (Arabic)', 'trim|xss_clean');
                ...
                foreach ($order_items as $row) {
                    if ($row['id'] == $consignment_item['order_item_id']) {
                        if ($row['pickup_location'] == $_POST['pickup_location'] && $row['seller_id'] == $_POST['seller_id']) {
                            ...
                            // Use Arabic product name if lang=ar and name_ar exists, else use product_name (already cleaned above)
                            $temp['name'] = ($lang == 'ar' && !empty($row['name_ar'])) ? $row['name_ar'] : $row['product_name'];
                            // Include Arabic product name field for API response
                            $temp['name_ar'] = (isset($row['name_ar']) && !empty($row['name_ar'])) ? $row['name_ar'] : '';
    ...
    public function create_order_consignment()
    {
        ...
        */
        ...
        if (!$this->form_validation->run()) {
            ...
            // Clean HTML tags from product names in order items for API response
            if (!empty($order_details['order_data'][0])) {
