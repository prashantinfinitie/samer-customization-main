...
*/
    ...
    public function get_categories()
    {
        /*
            ...
            lang:en/ar          // { default - en } optional - Language for response content
        */
        ...
        $this->form_validation->set_rules('lang', 'Language', 'trim|xss_clean|in_list[en,ar]');
        ...
        // Apply language transformation for Arabic (using private method for clean API response without HTML tags)
        if ($lang == 'ar' && !empty($cat_res)) {
            $cat_res = $this->apply_category_language($cat_res, $lang);
        }
        if ($lang == 'ar' && !empty($popular_categories)) {
            $popular_categories = $this->apply_category_language($popular_categories, $lang);
        }
    ...
    */
    ...
    public function get_products()
    {
        ...
        */
        ...
        if (!$this->form_validation->run()) {
            ...
            // Apply language transformation for Arabic (using private method for clean API response without HTML tags)
            if ($lang == 'ar' && !empty($products['product'])) {
                $products['product'] = $this->apply_product_language($products['product'], $lang);
            }
    ...
    public function get_slider_images()
    {
        ...
        foreach ($res as $row) {
            ...
            if (strtolower($res[$i]['type']) == 'categories') {
                ...
                // Apply language transformation for categories in slider
                $res[$i]['data'] = ($lang == 'ar' && !empty($cat_res)) ? $this->apply_category_language($cat_res, $lang) : $cat_res;
            } else if (strtolower($res[$i]['type']) == 'products') {
                $id = (!empty($res[$i]['type_id']) && isset($res[$i]['type_id'])) ? $res[$i]['type_id'] : '';
                $pro_res = fetch_product(NULL, NULL, $id);
                // Apply language transformation for products in slider
                $res[$i]['data'] = ($lang == 'ar' && !empty($pro_res['product'])) ? $this->apply_product_language($pro_res['product'], $lang) : $pro_res['product'];
            } else {
                $res[$i]['data'] = [];
            }
    ...
    public function place_order()
    {
        ...
        */
        ...
        // Shipping company fields (optional - only required when provider_type is 'company')
        $this->form_validation->set_rules('provider_type', 'Provider Type', 'trim|xss_clean');
        ...
        if (!$this->form_validation->run()) {
            ...
            if (!empty($product_delivarable) && ($product_type[0] == "variable_product" || $product_type[0] == "simple_product")) {
                ...
                if (!empty($product_not_delivarable)) {
                    ...
                    // Add shipping company quote snapshot - MUST be done BEFORE place_order()
                    if (
                ...
                // Add shipping company quote snapshot (for non-physical products if applicable)
                if (
                    isset($_POST['provider_type']) && $_POST['provider_type'] === 'company' &&
                    isset($_POST['selected_quote_id']) && !empty($_POST['selected_quote_id'])
                ) {
    ...
    public function get_orders()
    {
        ...
        if (!$this->form_validation->run()) {
            ...
            if (!empty($order_details)) {
                ...
                // Clean HTML tags from product names in order items for API response
                // fetch_orders() uses apply_locale_to_product() which adds HTML tags - we need clean text for APIs
                if (!empty($order_details['order_data'])) {
                    foreach ($order_details['order_data'] as &$order) {
                        if (isset($order['order_items']) && is_array($order['order_items']) && !empty($order['order_items'])) {
                            foreach ($order['order_items'] as &$item) {
                                // Remove HTML tags from product name if present (from apply_locale_to_product)
                                if (isset($item['name'])) {
                                    // If lang=ar and name_ar exists, use name_ar directly (clean text)
                                    if ($lang == 'ar' && !empty($item['name_ar'])) {
                                        $item['name'] = $item['name_ar'];
                                    } else {
                                        // Strip HTML tags but keep the text content (fallback)
                                        $item['name'] = strip_tags($item['name']);
                                    }
                                }
                                // Also handle category_name if it exists in order items
                                if (isset($item['category_name'])) {
                                    if ($lang == 'ar' && !empty($item['category_name_ar'])) {
                                        $item['category_name'] = $item['category_name_ar'];
                                    } else {
                                        $item['category_name'] = strip_tags($item['category_name']);
                                    }
                                }
                            }
                        }
  ...
  */
                ...
                for ($i = 0; $i < count($tmp_cart_user_data); $i++) {
                    ...
                    if (!empty($product_data[0]['product_id'])) {
                        ...
                        if (!empty($pro_details['product'])) {
                            ...
                            if (!empty($pro_details['product'])) {
                                // Apply language transformation for products in cart
                                $cart_user_data[$i]['product_details'] = ($lang == 'ar') ? $this->apply_product_language($pro_details['product'], $lang) : $pro_details['product'];
                            } else {
        ...
        */
        ...
        if (!$this->form_validation->run()) {
            ...
            if (!empty($res)) {
                ...
                // Apply language transformation for favorite products
                if ($lang == 'ar' && !empty($res1)) {
                    $res1 = $this->apply_product_language($res1, $lang);
                }
            } else {
                $this->response['error'] = true;
        ...
        if (!empty($sections)) {
            for ($i = 0; $i < count($sections); $i++) {
                ...
                if (!empty($products['product'])) {
                    ...
                    // Apply language transformation for products in sections
                    $sections[$i]['product_details'] = ($lang == 'ar' && !empty($products['product'])) ? $this->apply_product_language($products['product'], $lang) : $products['product'];
    ...
    */
    ...
    /**
     * Get Delivery Info for Checkout
     *
     * Returns delivery availability, provider type (delivery_boy or company),
     * and either delivery charges or shipping company quotes based on the address.
     *
     * This is the main endpoint for checkout delivery handling in the mobile app.
     */
    public function get_delivery_info()
    {
        /*
            address_id: 10  (required) - Customer's selected delivery address

            Response varies based on provider_type:
        ...
        */
        ...
        // Get address details
        $address_data = fetch_details('addresses', ['id' => $address_id, 'user_id' => $user_id], 'pincode, city');

        if (empty($address_data)) {
            $this->response['error'] = true;
            $this->response['message'] = "Invalid address or address does not belong to user.";
        ...
        // Check if zipcode exists and get provider_type
        $zipcode_data = fetch_details('zipcodes', ['zipcode' => $zipcode], 'id, provider_type, delivery_charges, minimum_free_delivery_order_amount');
        ...
        $provider_type = isset($zipcode_data[0]['provider_type']) ? $zipcode_data[0]['provider_type'] : 'delivery_boy';

        // Handle based on provider type
        if ($provider_type === 'company') {
            // Shipping Company Flow - Return quotes
            ...
            if (empty($quotes)) {
                $this->response['error'] = true;
                $this->response['message'] = "No shipping quotes available for this location.";
                $this->response['delivery_available'] = false;
            ...
            foreach ($quotes as $quote) {
                $additional_charges = [];
                if (!empty($quote['additional_charges'])) {
                    $additional_charges = is_string($quote['additional_charges'])
                        ? json_decode($quote['additional_charges'], true)
                        : $quote['additional_charges'];
                }

                $formatted_quotes[] = [
                    'id' => $quote['id'],
                    'shipping_company_id' => $quote['shipping_company_id'],
                    'company_name' => isset($quote['company_name']) ? $quote['company_name'] : '',
                    'company_email' => isset($quote['company_email']) ? $quote['company_email'] : '',
                    'company_phone' => isset($quote['company_phone']) ? $quote['company_phone'] : '',
                    'price' => floatval($quote['price']),
                    'cod_available' => isset($quote['cod_available']) ? intval($quote['cod_available']) : 1,
                    'estimated_days' => isset($quote['estimated_days']) ? $quote['estimated_days'] : '',
            ...
            $this->response['provider_type'] = 'company';
            $this->response['quotes'] = $formatted_quotes;
            print_r(json_encode($this->response));
            return;
        } else {
            // Delivery Boy Flow - Return standard delivery charges
            $shipping_method = get_settings('shipping_method', true);
            $cart_total_response = get_cart_total($user_id, '', 0, $address_id);
            $cart_total = isset($cart_total_response['sub_total']) ? floatval($cart_total_response['sub_total']) : 0;

            // Check for shiprocket
            if (isset($shipping_method['shiprocket_shipping_method']) && $shipping_method['shiprocket_shipping_method'] == 1) {
                $this->load->library(['Shiprocket']);

                // Get cart for parcels
                $cart_data = $this->cart_model->get_user_cart($user_id, 0);
                if (!empty($cart_data)) {
                    $parcels = make_shipping_parcels($cart_data);
                    $parcels_details = check_parcels_deliveriblity($parcels, $zipcode);

                    if (
                        isset($shipping_method['standard_shipping_free_delivery']) &&
                        $shipping_method['standard_shipping_free_delivery'] == 1 &&
                        $cart_total > $shipping_method['minimum_free_delivery_order_amount']
                    ) {

                        $this->response['delivery_charge_with_cod'] = 0;
                        $this->response['delivery_charge_without_cod'] = 0;
                    } else {
                        $this->response['delivery_charge_with_cod'] = isset($parcels_details['delivery_charge_with_cod'])
                            ? floatval($parcels_details['delivery_charge_with_cod']) : 0;
                        $this->response['delivery_charge_without_cod'] = isset($parcels_details['delivery_charge_without_cod'])
                            ? floatval($parcels_details['delivery_charge_without_cod']) : 0;
                    }

                    $this->response['estimate_date'] = isset($parcels_details['estimate_date']) ? $parcels_details['estimate_date'] : '';
                    $this->response['estimated_delivery_days'] = isset($parcels_details['estimated_delivery_days'])
                        ? $parcels_details['estimated_delivery_days'] : '';
                }
            } else {
                // Local delivery charges
                $delivery_charge = get_delivery_charge($address_id, $cart_total, $user_id);

                // Parse delivery charge (could be "with_cod,without_cod" format or single value)
                if (is_array($delivery_charge)) {
                    $this->response['delivery_charge_with_cod'] = isset($delivery_charge['delivery_charge_with_cod'])
                        ? floatval($delivery_charge['delivery_charge_with_cod']) : 0;
                    $this->response['delivery_charge_without_cod'] = isset($delivery_charge['delivery_charge_without_cod'])
                        ? floatval($delivery_charge['delivery_charge_without_cod']) : 0;
                } else {
                    $charge = floatval(str_replace(',', '', $delivery_charge));
                    $this->response['delivery_charge_with_cod'] = $charge;
                    $this->response['delivery_charge_without_cod'] = $charge;
                }

                // Calculate estimate date (e.g., 3-5 days from now)
                $estimate_days = isset($shipping_method['estimated_delivery_days']) ? $shipping_method['estimated_delivery_days'] : 5;
                $this->response['estimate_date'] = date('d-M-Y', strtotime("+{$estimate_days} days"));
            }

            $this->response['error'] = false;
            $this->response['message'] = "Delivery info retrieved successfully.";
            $this->response['delivery_available'] = true;
